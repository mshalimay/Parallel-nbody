// TWO ARRAYS AND SORT VIA PREFIX SUM
...
int* bin_prefix_sum_d_copy;     // Optionally, keep a copy of bin_prefix_sum_d
...
__global__ void count_particles_per_bin(particle_t* parts, int* bin_counts_d, int num_parts, double bin_size, int num_bins_x) {
...
__global__ void sort_particles_by_bin(particle_t* parts, particle_t* sorted_particles, int* sorted_to_parts,
                                                int* bin_prefix_sum_d, int num_particles, double bin_size, int num_bins_x) {
    int tid = threadIdx.x + blockIdx.x * blockDim.x;
    ...
    sorted_to_parts[tid] = idx;

    // initial: bin_prefix_sum = [0, 3, 4, 4, 6, 10]
    // final:   bin_prefix_sum = [3, 4, 4, 6, 10, 10]
}
    ...
    for (int drow = -1; drow <= 1; drow++) {
        for (int dcol = -1; dcol <= 1; dcol++) {
            ...
            if (neighbor_bin_x >= 0 && neighbor_bin_x < num_bins_x &&
                ...
                int start_idx = (neighbor_bin_idx == 0) ? 0 : bin_prefix_sum[neighbor_bin_idx - 1];
                int end_idx = bin_prefix_sum[neighbor_bin_idx];
...
void init_simulation(particle_t* parts, int num_parts, double size) {
    ...
    cudaMalloc((void**)&sorted_to_parts, num_parts * sizeof(int));

    // If using option 2 in simulate_one_step
    cudaMalloc((void**)&bin_prefix_sum_d_copy, memsize); 
...
__global__ void fill_parts(particle_t* parts, particle_t* sorted_particles, int* sorted_to_parts){
    int tid = threadIdx.x + blockIdx.x * blockDim.x;
    parts[tid] = sorted_particles[sorted_to_parts[tid]];
}
...
void simulate_one_step(particle_t* parts, int num_parts, double size) {
    ...
    sort_particles_by_bin<<<blks, NUM_THREADS>>>(parts, sorted_particles_d, sorted_to_parts, bin_prefix_sum_d, num_parts, bin_size, num_bins_x);
    
    // Compute forces
    compute_forces_gpu<<<blks, NUM_THREADS>>>(sorted_particles_d, bin_prefix_sum_d, num_parts, num_bins_x, bin_size);

    // Move particles
    move_gpu<<<blks, NUM_THREADS>>>(sorted_particles_d, num_parts, size);

    // map sorted_particles_d back to parts
    fill_parts<<<blks, NUM_THREADS>>>(parts, sorted_particles_d, sorted_to_parts);
}
